...
export class SearchCondition extends Component {
	...
	searchFunc() {
		...
		if (
			stateDate.getTime() > maxDate.getTime() ||
			stateDate.getTime() < current.getTime()
		) {
			alert(
				"Invalid Date! Search is limited to 3 days because of the API license."
			);
			return false;
		}
			...
			if (chosenWeather != null) {
				...
				for (let hour = 0; hour <= 23; hour++) {
					if (
						matchWeather(
							chosenWeather,
							forecast[index].hour[hour].condition.text
						)
					) {
					...
					{this.state.successfulSearch ? (
						this.state.searchList.map((hour) => (
							<Card>
								...
								{parseInt(hour.time.split("-")[1])} {hour.condition.text}{" "}
								{hour.time.split(" ")[1]}{" "}
							</Card>
						))
					) : (
